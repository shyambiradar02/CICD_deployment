...
# Load Data
dataset = pd.read_csv(r'C:\Users\Shyam\Desktop\CICD\Iris.csv')

# Feature names (Ensure no extra spaces or parentheses)
import re
dataset.columns = [re.sub(r'(?<=[a-z])(?=[A-Z])', '_', col).lower().replace('_cm', '').replace('cm', '') for col in dataset.columns]
...
# Encode species as numeric target and drop Id
dataset['target'] = pd.Categorical(dataset['species']).codes
